...
if ($originalVM.SecurityProfile){
   $securityType = $originalVM.SecurityProfile.SecurityType
   $secureboot = $originalVM.SecurityProfile.UefiSettings.SecureBootEnabled
   $vtpm = $originalVM.SecurityProfile.UefiSettings.VTpmEnabled
}
...
if ($securityType){
   $newVM = Set-AzVMSecurityProfile -VM $newVM -SecurityType $securityType
   $newVM = Set-AzVMUefi -VM $newVM -EnableVtpm $vtpm -EnableSecureBoot $secureboot
}
